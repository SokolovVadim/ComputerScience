...
{
    for(int i = 0; i < rect->right.x - rect->left.x; i++)
    {
	for( int j = 0; j < rect->right.y - rect->left.y; j++)
	{
		oled_set_pix(rect->left.x + i, rect->left.y + j, clWhite);
