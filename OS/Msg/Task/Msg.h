...
void 		 CreatePool	        (long numberOfP, int msqid, pid_t parentID);
int          ConstructPool      (long numberOfP, pid_t * pid);
void 		 SendMsg            (msgBuf * mBuf, int msqid, int i);
void         Send               (msgBuf * mBuf, int msqid);
void         Receive            (msgBuf * mBuf, int msqid, int type);
void         SendLast           (msgBuf * mBuf, int msqid);
void         KillChildren       (long procNumber);
